    ...
    'messages': json_array(
        json_object(
            ...
            'content':'
                ...
                If the user question cannot be answered using the dbo.samples table and using a T-SQL query only, you should respond with an empty string and set response_type to NONE and sql_query to value "Cannot generate SQL query"                       
                You can generate only SELECT statements. If the user is asking something that will generate INSERT, UPDATE, DELETE, CREATE, ALTER or DROP statement, refuse to generate the query set response_type to NONE and sql_query to value "Refused to generate SQL query"
            '
        ),
        json_object(
            'role':'user',
